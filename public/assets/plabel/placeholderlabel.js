/** Minified version of *detect-autofill* */
...
class PlaceholderLabel {
    constructor(o) {
        this.o = this._m({
            s: 'form',                      // selector
            e: 'pl-none',                   // excludeClass -> use this class to not initialize p-label on forms
            a: true,                        // detect autofill via "detect-autofill" plugin
            i: {                            // input fields
                p: 'div.widget',            // parent
                l:  'label',                // label
                i: [                        // includes of input type
                    'input',
                    'select',
                    'textarea'
                ],
                e: [                        // excludes of input type
                    '[type="checkbox"]',
                    '[type="radio"]',
                    '[type="range"]',
                    '[type="hidden"]',
        ...
        this.f = document.querySelectorAll(this.o.s)

        if(!this.f.length)
            return

        this.c = {
            i: 'pl-init',                   // init class
            p: 'pl-parent',                 // parent class
            f: '_focus',                    // focus class
            b: '_blur',                     // blur class
            c: '_changed',                  // changed class
            l: '_filled'                    // filled class
        }
    ...
    /**
     * Initialization of placeholder label
     */
    _i()
    {
        this.b = []
        ...
        for(let f of this.f)
        {
            if(!!this.o.e && f.classList.contains(this.o.e))
                return
            ...
            let p = f.querySelectorAll(this.o.i.i.join(','))

            if(!p.length)
                continue

            let a = [...p].filter(i => !i.matches(this.o.i.e.join(',')))

            a.forEach((i) => {
                this._r(f, i) // registerInputEvents
                this._u(i)    // update
            })
    ...
    /**
     * Registering the input events
     */
    _r(f, i)
    {
        let c = new Event('change');

        // Consider autocomplete and dispatch changeEvent
        ...
        // Add label and parent
        let p = i.closest(this.o.i.p)
        p.classList.add(this.c.p)

        i.phl = {
            l: p.querySelector(this.o.i.l),
            p: p
        }
...
document.addEventListener('DOMContentLoaded', function () {
    window.pl = new PlaceholderLabel()
});
